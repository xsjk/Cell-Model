...
from .disk import get_alphas, get_bessel_basis, get_legendre_basis
...
def transform(func, R, L, M, K_r, K_z, N_r=60, N_theta=200, N_z=60, method: Literal["legendre", "simpson"] = "legendre"):
    ...
    coeffs /= (np.pi * L * (R**2) / 2.0) * (sp.jv(m[:, None, None] + 1, alphas[:, :, None]) ** 2)  # (2M+1, K_r, K_z)
    return coeffs
...
def transform_fast(func, R, L, M, K_r, K_z, N_r=60, N_theta=200, N_z=60, method: Literal["legendre", "simpson"] = "legendre"):
    ...
    alphas = get_alphas(M, K_r)  # (2M+1, K_r)
    coeffs /= (np.pi * L * (R**2)) * (sp.jv(m[:, None, None] + 1, alphas[:, :, None]) ** 2)  # (2M+1, K_r, K_z)
