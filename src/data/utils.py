...
def to_mp4(frames: np.ndarray, output_path: str, fps: int = 30, crf: int = 30, preset: str = "veryslow", slient: bool = True):
    ...
    proc = subprocess.run(
        ...
        stdout=subprocess.DEVNULL if slient else None,
        stderr=subprocess.DEVNULL if slient else None,
    )
    # fmt: on
...
def get_bounding_box(mask, axes=[-3, -2, -1], even=True) -> tuple[slice, ...]:
    ...
    When multiple objects are present, the max bounding box will be returned.
    """
